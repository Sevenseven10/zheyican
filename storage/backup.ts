...
export const createBackupPart = async (_source: BackupSource, _plan: { backupId: string; exportedAt: string; parts: BackupPartPlan[] }, _partIndex: number): Promise<{ blob: Blob; file: File; filename: string }> => unavailable();
export const saveBackupPart = async (_file: File): Promise<void> => unavailable();
