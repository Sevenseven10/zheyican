...
function History({ meals, onEdit, onDataBackup }: { meals: Meal[]; onEdit: (meal: Meal) => void; onDataBackup: () => void }) {
  ...
  return <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
    ...
    <Pressable accessibilityRole="button" accessibilityLabel="数据与备份" onPress={onDataBackup} style={styles.dataBackupEntry}><Text style={styles.dataBackupEntryText}>数据与备份</Text><Text style={styles.dataBackupEntryHint}>为以后的记录留一份副本</Text></Pressable>
  </ScrollView>;
}
...
function DataBackup({ meals, onBack }: { meals: Meal[]; onBack: () => void }) {
  ...
  if (detail) {
    ...
    return <ScrollView contentContainerStyle={styles.dataPage} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.preparing}><Text style={styles.preparingText}>{isBackup ? '备份功能准备中' : '恢复功能准备中'}</Text></View>
    </ScrollView>;
  }
  return <ScrollView contentContainerStyle={styles.dataPage} showsVerticalScrollIndicator={false}>
