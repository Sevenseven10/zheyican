...
async function readMeals() {
  ...
  return rows.map((row) => ({ ...row, photos: (JSON.parse(row.photos) as Array<string | Partial<PhotoComposition>>).map(normalizePhoto) }));
}
...
const imageDimensions = (uri: string) => new Promise<PhotoDimensions>((resolve) => Image.getSize(uri, (width, height) => resolve({ width, height }), () => resolve({ width: 1, height: 1 })));

function ComposedPhoto({ photo, frameWidth, frameHeight }: { photo: PhotoComposition; frameWidth: number; frameHeight: number }) {
  const [measured, setMeasured] = useState<PhotoDimensions>({ width: photo.originalWidth, height: photo.originalHeight });
  useEffect(() => { if (measured.width <= 0 || measured.height <= 0) imageDimensions(photo.uri).then(setMeasured); }, [photo.uri, measured.width, measured.height]);
  const layout = getComposedImageLayout(photo, frameWidth, frameHeight, measured);
  return <Image
...
function PhotoComposer({ photo, onCancel, onDone }: { photo: PhotoComposition; onCancel: () => void; onDone: (photo: PhotoComposition) => void }) {
  const { width: windowWidth } = useWindowDimensions();
  const frameWidth = windowWidth - 40;
  const frameHeight = Math.min(430, frameWidth * 1.12);
