...
function PhotoGrid({ photos }: { photos: string[] }) {
  ...
  return <View key={`photo-layout-${count}`} style={[styles.photoGrid, { height: layout.height }]}>
    {photos.map((uri, index) => {
      ...
      return <View key={uri} style={[styles.mealPhotoFrame, frame]}>
        <Image source={{ uri }} resizeMode={resizeMode} style={styles.mealPhoto} />
      </View>;
    })}
  </View>;
}
...
function AddMeal({ meal, onCancel, onSave }: { meal: Meal | null; onCancel: () => void; onSave: () => void }) {
  const [photos, setPhotos] = useState<string[]>(meal?.photos ?? []); const [food, setFood] = useState(meal?.foodText ?? ''); const [note, setNote] = useState(meal?.note ?? ''); const [type, setType] = useState<MealType>(meal?.mealType ?? defaultType()); const [cameraOpen, setCameraOpen] = useState(false); const [permission, requestPermission] = useCameraPermissions(); const camera = useRef<CameraView>(null); const newPhotoUris = useRef(new Set<string>()); const initialPhotoUris = useRef(new Set(meal?.photos ?? []));
  ...
  const persistPhotos = async (uris: string[]) => {
    ...
    stored.forEach((uri) => newPhotoUris.current.add(uri));
