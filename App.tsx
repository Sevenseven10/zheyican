...
import {
  Alert, Image, Modal, Platform, Pressable, ScrollView, StyleSheet, Text,
  TextInput, View,
} from 'react-native';

type MealType = '早餐' | '午餐' | '晚餐' | '加餐';
type Meal = { id: string; createdAt: string; mealDate: string; mealTime: string; mealType: MealType; photos: string[]; foodText: string; note: string | null };
...
async function readMeals() {
  ...
  return rows.map((row) => ({ ...row, photos: JSON.parse(row.photos) }));
}
...
function PhotoGrid({ photos, dark = false }: { photos: string[]; dark?: boolean }) {
  const count = photos.length;
  if (!count) return null;
  return <View style={[styles.photoGrid, count === 1 && styles.onePhoto, count === 2 && styles.twoPhotos, count === 3 && styles.threePhotos, count === 4 && styles.fourPhotos, count >= 5 && styles.fivePhotos]}>
    {photos.map((uri, i) => <Image key={`${uri}-${i}`} source={{ uri }} style={[styles.mealPhoto, count === 1 && styles.fullPhoto, count === 3 && i === 0 && styles.featuredPhoto, count >= 5 && styles.smallGridPhoto]} />)}
  </View>;
}
...
export default function App() {
  ...
  if (screen === 'add') return <AddMeal onCancel={() => go('today')} onSave={async () => { await refresh(); go('today'); }} />;
  return <View style={styles.app}><StatusBar style="dark" />
    {screen === 'today' ? <Today meals={today} onAdd={() => go('add')} /> : <History meals={meals} />}
    <Nav screen={screen} go={go} />
  </View>;
}

function Today({ meals, onAdd }: { meals: Meal[]; onAdd: () => void }) {
  return <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
    <View style={styles.topline}><Text style={styles.wordmark}>吃过什么</Text><View style={styles.todayMark}><View style={styles.dot} /><Text style={styles.caption}>今天</Text></View></View>
    ...
    {meals.length === 0 ? <View style={styles.empty}><Text style={styles.emptyTitle}>今天还没有留下照片</Text><Text style={styles.emptyCopy}>吃完这一餐，再慢慢记下来。</Text><Pressable onPress={onAdd} style={styles.emptyAction}><Text style={styles.emptyActionText}>添加第一餐</Text></Pressable></View> : meals.map((meal) => <MealItem key={meal.id} meal={meal} />)}
  </ScrollView>;
}

function MealItem({ meal }: { meal: Meal }) {
  return <View style={styles.mealItem}><Text style={styles.mealMeta}>{meal.mealType} · {meal.mealTime}</Text><PhotoGrid photos={meal.photos} /><Text style={styles.foodText}>{meal.foodText}</Text>{meal.note ? <Text style={styles.note}>{meal.note}</Text> : null}<View style={styles.rule} /></View>;
}

function History({ meals }: { meals: Meal[] }) {
  ...
  return <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false}>
    <Text style={styles.wordmark}>吃过什么</Text><Text style={styles.historyTitle}>往前翻</Text><View style={styles.rule} />
    {!groups.length ? <View style={styles.empty}><Text style={styles.emptyTitle}>还没有过去的餐次</Text><Text style={styles.emptyCopy}>从今天开始，慢慢留下一卷生活。</Text></View> : groups.map(({ date, meals: daily }) => <View key={date} style={styles.dayGroup}><View style={styles.dayHeading}><Text style={styles.dayNumber}>{date.slice(5).replace('-', '.')}</Text><Text style={styles.caption}>{weekLabel(date)}</Text></View>{daily.map((meal) => <View key={meal.id} style={styles.historyMeal}><PhotoGrid photos={meal.photos} /><Text style={styles.historyCaption}>{meal.mealType} · {meal.mealTime}　{meal.foodText}</Text></View>)}<View style={styles.rule} /></View>)}
  </ScrollView>;
}

function AddMeal({ onCancel, onSave }: { onCancel: () => void; onSave: () => void }) {
  const [photos, setPhotos] = useState<string[]>([]); const [food, setFood] = useState(''); const [note, setNote] = useState(''); const [type, setType] = useState<MealType>(defaultType()); const [cameraOpen, setCameraOpen] = useState(false); const [permission, requestPermission] = useCameraPermissions(); const camera = useRef<CameraView>(null);
  const persistPhotos = async (uris: string[]) => {
    ...
    const stored = await Promise.all(uris.map(async (uri) => { const ext = uri.split('.').pop()?.split('?')[0] || 'jpg'; const target = `${root}${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`; await FileSystem.copyAsync({ from: uri, to: target }); return target; }));
  ...
  const save = async () => { if (!photos.length) return Alert.alert('先放一张照片', '一餐至少需要一张照片。'); if (!food.trim()) return Alert.alert('写下吃了什么', '用一句话留住这一餐。'); const now = new Date(); await saveMeal({ id: `${now.getTime()}-${Math.random().toString(36).slice(2)}`, createdAt: now.toISOString(), mealDate: dateKey(now), mealTime: timeKey(now), mealType: type, photos, foodText: food.trim(), note: note.trim() || null }); onSave(); };
  ...
  return <View style={styles.addApp}><StatusBar style="light" /><ScrollView contentContainerStyle={styles.addPage} keyboardShouldPersistTaps="handled">
    <View style={styles.addHeader}><Pressable onPress={onCancel}><Text style={styles.addCancel}>取消</Text></Pressable><Text style={styles.addTitle}>添加一餐</Text><View style={{ width: 32 }} /></View>
    <View style={styles.addPhotoSection}>{photos.length ? <View style={styles.editorGrid}>{photos.map((uri, index) => <View key={uri} style={styles.editPhoto}><Image source={{ uri }} style={styles.editImage} /><View style={styles.photoControls}><Pressable hitSlop={6} onPress={() => move(index, -1)}><Text style={styles.photoControl}>←</Text></Pressable><Pressable hitSlop={6} onPress={() => setPhotos((p) => p.filter((_, i) => i !== index))}><Text style={styles.remove}>×</Text></Pressable><Pressable hitSlop={6} onPress={() => move(index, 1)}><Text style={styles.photoControl}>→</Text></Pressable></View></View>)}</View> : <Text style={styles.photoLead}>先拍下这一餐</Text>}
