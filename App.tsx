...
function AddMeal({ meal, onCancel, onSave }: { meal: Meal | null; onCancel: () => void; onSave: () => void }) {
  ...
  const openCamera = async () => { const cameraPermission = permission?.granted ? permission : await requestPermission(); if (!cameraPermission.granted) return Alert.alert('需要相机权限', '开启权限后即可拍下这一餐。'); setCameraReady(false); setCameraOpen(true); };
  const closeCamera = () => { setCameraOpen(false); setCameraReady(false); };
  const capture = async () => { if (!cameraReady) return; const photo = await camera.current?.takePictureAsync({ quality: 1 }); if (photo) { await persistPhotos([photo]); closeCamera(); } };
  ...
  if (cameraOpen && permission?.granted) return <Modal animationType="slide" presentationStyle="fullScreen"><View style={styles.cameraWrap}><CameraView ref={camera} style={styles.cameraPreview} facing="back" active={cameraOpen} mode="picture" onCameraReady={() => setCameraReady(true)} onMountError={({ message }) => { setCameraReady(false); Alert.alert('相机无法启动', message); }} /><Pressable onPress={closeCamera} style={styles.cameraCancel}><Text style={styles.cameraText}>取消</Text></Pressable><Pressable disabled={!cameraReady} onPress={capture} style={[styles.shutter, !cameraReady && styles.shutterDisabled]}><View /></Pressable></View></Modal>;
