...
import { getMealPhotoLayout, PhotoDimensions } from './photoLayout';
...
function PhotoGrid({ photos }: { photos: string[] }) {
  ...
  const [dimensions, setDimensions] = useState<Record<string, PhotoDimensions>>({});

  useEffect(() => {
    let active = true;
    photos.forEach((uri) => {
      if (dimensions[uri]) return;
      Image.getSize(uri, (width, height) => {
        if (active) setDimensions((current) => current[uri] ? current : { ...current, [uri]: { width, height } });
      }, () => undefined);
    });
    return () => { active = false; };
  }, [photos, dimensions]);
  ...
  const layout = getMealPhotoLayout(count, containerWidth, photos.map((uri) => dimensions[uri]));
  return <View key={`photo-layout-${count}`} style={[styles.photoGrid, { height: layout.height }]}>
    {photos.map((uri, index) => {
      const { resizeMode, ...frame } = layout.frames[index];
      return <Image key={uri} source={{ uri }} resizeMode={resizeMode} style={[styles.mealPhoto, frame]} />;
    })}
  </View>;
}
