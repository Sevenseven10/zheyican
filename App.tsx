...
import { getComposedImageLayout, hasIntrinsicDimensions, normalizePhoto, normalizeRotation } from './photoLayout';
...
function ComposedPhoto({ photo, frameWidth, frameHeight, onDimensionsResolved }: { photo: PhotoComposition; frameWidth: number; frameHeight: number; onDimensionsResolved?: (dimensions: PhotoDimensions) => void }) {
  ...
  const rotation = normalizeRotation(safePhoto.rotation);
  if (rotation === 0) {
    return <Image
      source={{ uri: resolvedUri }}
      resizeMode="cover"
  ...
  return (
    <View style={{ position: 'absolute', left: layout.left, top: layout.top, width: layout.width, height: layout.height, overflow: 'hidden' }}>
      <Image
        source={{ uri: resolvedUri }}
        resizeMode="cover"
        style={{
          position: 'absolute',
          left: swapped ? (layout.width - layout.height) / 2 : 0,
          top: swapped ? (layout.height - layout.width) / 2 : 0,
          width: swapped ? layout.height : layout.width,
          height: swapped ? layout.width : layout.height,
...
function PhotoComposer({ photo, onCancel, onDone, onDimensionsResolved }: { photo: PhotoComposition; onCancel: () => void; onDone: (photo: PhotoComposition) => void; onDimensionsResolved: (dimensions: PhotoDimensions) => void }) {
  ...
  return <View style={[styles.composerApp, platformLayout.composerApp]}><StatusBar style="light" />
    ...
    <View style={styles.composerStage}><PhotoGestureSurface enabled={hasIntrinsicDimensions(draft)} frameWidth={frameWidth} frameHeight={frameHeight} nativeHandlers={panResponder.panHandlers} onChange={updateDraft} photo={draft} style={[styles.composerFrame, { width: frameWidth, height: frameHeight }]}><ComposedPhoto photo={draft} frameWidth={frameWidth} frameHeight={frameHeight} onDimensionsResolved={resolveDraftDimensions} /></PhotoGestureSurface><View style={styles.composerControls}><Pressable hitSlop={12} onPress={() => updateDraft({ ...draftRef.current, rotation: normalizeRotation((draftRef.current.rotation ?? 0) + 1) })} style={styles.composerRotate}><Text style={styles.composerRotateText}>旋转 90°</Text></Pressable><Text style={styles.composerHint}>双指缩放 · 拖动调整位置</Text></View></View>
  </View>;
}
