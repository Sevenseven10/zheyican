...
async function run() {
  let number = 0; let urls = 0;
  const repo = createIndexedDbRepositories({ databaseName: `${WEB_DATABASE_NAME}-lifecycle-${Date.now()}`, indexedDb: indexedDB, createPhotoId: () => `p-${++number}`, createObjectUrl: () => `blob:${++urls}`, revokeObjectUrl: () => undefined });
